...
1. **[Microsoft Copilot Studio へようこそ]** ページにダイアログが表示されたら、国/地域を選択してから **[はじめに]** を選択します。
...
1. **[説明]** フィールドに、「`A product support agent that can answer queries about Contoso Electronics products`」と入力します。
...
1. 推奨プロンプトは生成 AI で生成されていることにご注意ください。**[推奨プロンプト]** セクションは今は空欄のままにしておいてください。 これらのプロンプトは、今後の演習で更新します。
...
1. エージェントの概要情報の右側に **[エージェントのテスト]** ウィンドウが表示されない場合は、**[公開]** ボタンの横にある **[テスト]** ボタンを選択して、テスト ペインを開きます。
1. テスト ペイン内のプロンプト ボックスに「`What can you do?`」と入力し、メッセージを送信します。
1. 応答を待ちます。 応答が "Copilot エージェントをご利用いただき、ありがとうございます。" というテキストで始まることに注目してください。 これは、前に定義したエージェントへの指示どおりです。

    ![テスト ウィンドウでの Product support エージェントとの会話のスクリーンショット。](../Media/product-support-test-pane.png)
...
Web ブラウザーで引き続き、**コンテキスト内**エクスペリエンスをテストしてみましょう。
...
1. プロンプト ボックスに、<kbd>@</kbd> 記号を入力します。 利用可能なエージェントのリストのポップアップが表示されます ![Microsoft 365 Copilot にエージェントのポップアップが表示されている Microsoft Edge のスクリーンショット。](../Media/copilot-agents-flyout.png)

1. ポップアップで **[Product Support]** を選択します。 これで、Copilot との会話内で Product Support エージェントと**コンテキストを踏まえて**チャットしています。つまり、エージェントは Copilot との会話のコンテキストを考慮できます。
...
1. 応答を待ちます。 応答が "Copilot エージェントをご利用いただき、ありがとうございます。" というテキストで始まることに注目してください。 これは、エージェントへの指示で指定したガイダンスに従っています。
