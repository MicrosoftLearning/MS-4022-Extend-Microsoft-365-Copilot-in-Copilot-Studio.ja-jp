...
1. **説明**フィールドに、`A product support agent that can answer queries about Contoso Electronics products` を入力します。
...
1. 提案されたプロンプトが、生成 AI を使用して生成されていることにご注意ください。 **[推奨されるプロンプト]** セクションは空のままにします。 これらのプロンプトは、今後の演習で更新します。
...
> [!NOTE]
> エージェントを編集する必要がある場合は、エージェントの概要ページの **[詳細]** セクションで **[編集]** を選択します。 変更を保存。 もう一度テストする前に、テスト ペイン内の **[新しいチャット]** ボタンを選択します。
...
1. プロンプト ボックスに、<kbd>@</kbd> 記号を入力します。 使用可能なエージェントの一覧を示すポップアップが表示されます。
