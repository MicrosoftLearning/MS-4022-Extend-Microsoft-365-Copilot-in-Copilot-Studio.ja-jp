...
lab:
  ...
  description: この演習では、前の演習で作成した宣言型エージェントを更新してカスタム指示とグラウンディング データを追加します。
  duration: 20 minutes
  level: 200
  islab: true
---
...
この演習では、前の演習で作成した宣言型エージェントを更新してカスタム指示とグラウンディング データを追加します。 
...
1. **生ファイルのダウンロード** ボタンを選択して、**Products.zip** をダウンロードします。
    ...
    ![GitHub の生ファイル ダウンロード ボタンが強調表示されている Microsoft Edge のスクリーンショット。](../Media/download-raw-file.png)

1. ダウンロードしたフォルダーを**開き**、**[すべて展開]** を選択して、`Products` という名前の、コンピューター上の新しいフォルダーにコンテンツを抽出します。このフォルダーには後でアクセスできます。

### SharePoint サイトを作成する

1. Web ブラウザーで、[https://m365.cloud.microsoft](https://m365.cloud.microsoft/chat) に移動し、このラボに使用している Microsoft 365 アカウントで**サインイン**します。 
1. 左側のメニューで **[アプリ]** (グリッド アイコン) を選択し、[すべてのアプリ] を選択して Microsoft 365 アプリのメニューを開きます。

    ![Copilot Chat の [M365 アプリ] ボタンのスクリーンショット。](../Media/apps-icon.png)
1. アプリのカタログから **[SharePoint]** を選択します。
1. 左側のメニューから、**[サイトの作成]** を選択します。
...
1. **[完了]** を選びます。 ブラウザーで新しい SharePoint サイトに移動します。
...
1. **[Web アドレス]** プロパティを見つけます。 **SharePoint サイトの URL** は、Web アドレスの `https://DOMAIN.sharepoint.com/sites/SITE_NAME/LIBRARY_NAME` 形式の部分です。 URL は `https://DOMAIN.sharepoint.com/sites/ProductSupport/Products` 形式にする必要があります。ここでドメインは Microsoft 365 テナント ドメインです。
...
1. Copilot Studio でエージェントの概要ページの **[エージェントのテスト]** ペインから、**[新しいテスト セッションの開始]** ボタンを選択して、テスト ペインを最新の情報に更新します。
